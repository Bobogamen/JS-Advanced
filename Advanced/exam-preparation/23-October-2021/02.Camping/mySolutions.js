class SummerCamp {
    ...
    registerParticipant(name, condition, money) {
        ...
        let founderPerson = this.listOfParticipants.find(p => p.name == name);
    ...
    timeToPlay(typeOfGame, participant1, participant2) {
        if (typeOfGame == "Battleship" && participant1 && !participant2) {
            let founderPerson = this.listOfParticipants.find(p => p.name == participant1);

            if (!founderPerson) {
                throw new Error(`Invalid entered name/s.`);
            }

            founderPerson.power += 20;

            return `The ${participant1} successfully completed the game ${typeOfGame}.`

        } else if (typeOfGame == "WaterBalloonFights" && participant1 && participant2) {
    ...
    toString() {
        ...
        sortedParticipants.forEach(p => {
            message.push(`${p.name} - ${p.condition} - ${p.power} - ${p.wins}`)
        })
